...
- ◐ **3.3** Work area (Candasnos) + **Natura 2000** — legend with site **ID, name, hectares**; main cities; river. ~1:50,000. **DATA DONE** → `03_PROCESSED/natura2000_box.fgb` (18 sites by designation) + **auto-ha legend** `00_ADMIN/natura2000_legend.csv` (incl. Valcuerna 35,338 ha). **Towns** (`towns_33.fgb`, 67) + **AOS frame** (`aos_frame.fgb`) now added; river = 4.1 overlay. [C data+ha ✓ / K style]
